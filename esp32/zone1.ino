/*
  AgriVision AI - ESP32 Zone 1 Sensor Node
  =========================================
...
#define MQTT_CLIENT_ID "AgriVision-Zone1"
...
#define TOPIC_SENSORS "agrivision/zone1/sensors"
#define TOPIC_STATUS  "agrivision/zone1/status"
...
void tryConnectMQTT() {
  ...
  // Last Will: broker marks device offline if connection is lost
...
float readSoilPercent() {
  ...
  Serial.printf("[SOIL] Zone 1 Raw ADC: %d -> %.1f%%\n", soilRaw, soilPercent);
...
void setup() {
  ...
  // TLS
  // Keeping your current testing configuration.
  ...
  Serial.println("[SYSTEM] Zone 1 ready");
}
...
void loop() {
  ...
  Serial.printf("[MQTT] Zone 1 TX: %s\n", payload);
